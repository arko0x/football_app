...
def get_standings_for_league_and_season(league, season):
    ...
    list = [(""+str(i), "Real Madrid", str(random.randint(5, 20)), "40", "10", "20", "30", "100", "50", "50") for i in range(24)]
    return list
...
def get_fixtures_for_next_week(league, season):
    ...
    # url = "https://v3.football.api-sports.io/fixtures?from=" + current_date + "&to=" + current_date_plus_seven_days + "&league=" + str(league) + "&season=" + str(season)
    # fixtures = []
    #
    # response = requests.request("GET", url, headers=headers, data=payload)
    # data = json.loads(response.text)["response"]
